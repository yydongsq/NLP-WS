...
public class HanLPWSController {
    ...
    public Result<String> hanLPWS(@RequestParam(value = "dataSetId",required = false) String dataSetId,@RequestBody TbNlpDataset tbNlpDataset) {
        HanLPWSCommon ws = new HanLPWSCommon();
        String dataSetText = WsUtils.getRequestDataSet(dataSetId, tbNlpDataset);
        if(dataSetText == ""){
            return Result.error("未找到对应数据",null);
        }
        String hanlpWs = ws.getHanLPWSByMyAPI(dataSetText);
        log.info("HanLP调用成功");
